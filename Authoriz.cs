...
namespace Shop_10;
internal static class Authoriz
{
    ...
    public static bool Read(int longe)
    {
        ...
        switch (Control.ymarg - 1)
        {
            case 1:
                ...
                Console.SetCursorPosition(0, Control.ymarg);
                break;
            case 2:
            ...
            case 3:
                ...
                {
                    if (checuser.login == login && checuser.password == password)
                    {
                        role = checuser.role;
                        List<checku> employees = Serialize.desteruser<List<checku>>(Startname.Employee);
                        foreach (checku employee in employees)
                            if (checuser.id == employee.ideq)
                                username = employee.sname + " " + employee.name + " " + employee.otechestvo;
                        return false;
                    }
    ...
    private static string ReadPasword()
    {
        ...
        Console.SetCursorPosition(0, 2 + Control.ymarg - 1);
        lastconter = conter;
        return pass;
    }
}